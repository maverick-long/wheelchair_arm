...
pcl::PCLPointCloud2Ptr PointCloudReceiver::updatePointCloud(){
	...
	// if(currentCloud->header.frame_id=="l_foot")
	// 	return currentCloud;
	...
	// try
	// {
	// 	listener_.lookupTransform("l_foot", currentCloud->header.frame_id, ros::Time().fromNSec(currentCloud->header.stamp*10e2), transform);
